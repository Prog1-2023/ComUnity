...
Vector2f ComputeNormal(const float& _X , const float& _Y)
{
    const Vector2f& _normal = Vector2f(-_Y, _X);
    const float _norme = Length(_normal);
    return Vector2f((_normal.x / _norme) , (_normal.y / _norme));
}
