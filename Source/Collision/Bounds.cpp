...
RectangleBoundsData::RectangleBoundsData(const Vector2f& _position, const Vector2f& _size, const float& _rotation) : BoundsData(_position)
{
	size = _size;
	rotation = _rotation;
}
...
{
	//if (!((float)_data->rotation.asDegrees() % 90))
	//{
	//	//return FloatRect(_data->position - _data->size / 2.0f, _data->size).contains(_point);
	//	return true;
	//}

	if (!(float)degrees(_data->rotation ) % 90)
	{
		return _data->position - _data->size / 2.0f == _point.y || _point.x;
	}
...
Vector2f Bounds::Around(Vector2f _point, RectangleBoundsData* _data) const
{
	...
	const float _angle = radians(_data->rotation);
...
//METHODS NOT FINISH OR REMOVE METHODS
void Bounds::UpdateBounds(Actor* _actor)
{
	//TODO Implement Mesh Component 


	if (StaticMeshComponent* _meshComponent = _actor->GetComponent<StaticMeshComponent>())
	{
		const Vector3f& _pos = _meshComponent->GetOwner()->GetPosition();
		aiMesh* _mesh;
