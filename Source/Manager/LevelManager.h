...
public:
	...
	FORCEINLINE bool SetLevel(const string& _name)
	{
		...
		{
			//Assert(Fatal, "Level \"" + _name + "\" not found !");
			string _errorTxt = "Level /" + _name + "/ not found !";
			Assert(!allLevels.contains(_name), _errorTxt.c_str());
			return false;
		}
	...
	FORCEINLINE void UpdateCurrentLevel(float _deltaTime)
	{
